...
class TermTest {
    ...
    public void testSetAll() {
        ...
        for(int i = 0; i < EXPONENTS.length; i++) {
            ...
            test.setAll(c, e);
            //assertTrue(test.getCoefficient() == c && test.getExponent() == e); //This test doesn't compile
            assertEquals("Expected and actual coefficients DON'T match", c, test.getCoefficient());
            assertEquals("Expected and actual exponents DON'T match", e, test.getExponent());
        }
    }
    ...
    public void testToString() {
        ...
        for(int i = 0; i < TERMS.length; i++) {
            c = COEFFICIENTS[i];
            e = EXPONENTS[i];
            expected = (String) TERMS[i];

            test.setAll(c, e);
            actual = test.toString();
            actual = actual.replaceAll(" ", ""); //more flexible test doesnt depend on white space

            assertEquals("Expected and actual toStrings DON'T match", expected, (Object) actual);
        }
    }
